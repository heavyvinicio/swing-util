 ...
 * {@link JTable#setModel(javax.swing.table.TableModel)} {@link JTable}. </b>
 *
 * @author pocho
...
public class JTableModel<T> extends AbstractTableModel implements Serializable {
	...
	private void cargarDatosEditables() {
		if (this.listaDatos != null && !this.listaDatos.isEmpty()) {
			Integer[] columnEdit = ReflectionUtil.getBooleanFields(listaDatos.get(0), atributos);
			if (columnEdit != null && columnEdit.length > 0) {
				cargarDatosEditables(atributos, columnEdit);
				editableLock = true;
			}
		}
	}

	public JTableModel(List<T> listaDatos, String[] atributos, String[] nombreColumnas, Integer[] columnasEditables) {
		this();
		if (atributos.length != nombreColumnas.length) {
			Logger.getLogger(getClass().getName()).info("El número de atributos no coincide con el numero de columnas");
			return;
		...
		Integer[] columnEdit = ReflectionUtil.getBooleanFields(listaDatos.get(0), atributos);
		cargarDatosEditables(atributos, columnasEditables, columnEdit);
		this.columnNames.addAll(Arrays.asList(nombreColumnas));
		rows = ReflectionUtil.convertListData2Map(listaDatos, atributos);
		if (rows.isEmpty()) {
			return;
	...
	private void cargarDatosEditables(String[] atributos, Integer[] columnas) {

		List<Integer> listaTmp = Arrays.asList(columnas);
		columnasEditables = new HashMap<>();
		for (Integer index : listaTmp) {
			columnasEditables.put(index, atributos[index]);
		}
	}

	private void cargarDatosEditables(String[] atributos, Integer[] columnas, Integer[] columnEdit) {

		List<Integer> listaTmp = Arrays.asList(columnas);
		List<Integer> listaTmp2 = Arrays.asList(columnEdit);
		List<Integer> lista = new ArrayList<>();
		lista.addAll(listaTmp);
		lista.addAll(listaTmp2);
	...
	private boolean validateEditColumn(T obj, int columnIndex) {
		...
		if (validateEditColumn(columnIndex)) {
			return true;
		}
		List<Boolean> lista = ReflectionUtil.isCellEditable(obj);
